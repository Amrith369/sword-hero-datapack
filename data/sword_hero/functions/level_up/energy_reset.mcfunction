#ROPE
scoreboard players add @a[scores={take_energy=1..,ropeprof=1..},nbt={SelectedItem:{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{CustomModelData:3690012}}}] sword_ep 20
scoreboard players remove @a[scores={take_energy=1..,ropeprof=1..},nbt={SelectedItem:{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{CustomModelData:3690012}}}] ropeprof 1
scoreboard players set @a[scores={take_energy=1..,ropeprof=0},nbt={SelectedItem:{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{CustomModelData:3690012}}}] take_energy 0
scoreboard players add @a[scores={give_energy=1..,sword_ep=20..,ropeprof=..1000},nbt={SelectedItem:{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{CustomModelData:3690012}}}] ropeprof 1
scoreboard players remove @a[scores={give_energy=1..,sword_ep=20..},nbt={SelectedItem:{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{CustomModelData:3690012}}}] sword_ep 20
scoreboard players set @a[scores={give_energy=1..,sword_ep=..19},nbt={SelectedItem:{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{CustomModelData:3690012}}}] give_energy 0
#SIDERITE
scoreboard players add @a[scores={take_energy=1..,sideriteprof=1..},nbt={SelectedItem:{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{CustomModelData:3690013}}}] sword_ep 20
scoreboard players remove @a[scores={take_energy=1..,sideriteprof=1..},nbt={SelectedItem:{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{CustomModelData:3690013}}}] sideriteprof 1
scoreboard players set @a[scores={take_energy=1..,sideriteprof=0},nbt={SelectedItem:{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{CustomModelData:3690013}}}] take_energy 0
scoreboard players add @a[scores={give_energy=1..,sword_ep=20..,sideriteprof=..1000},nbt={SelectedItem:{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{CustomModelData:3690013}}}] sideriteprof 1
scoreboard players remove @a[scores={give_energy=1..,sword_ep=20..},nbt={SelectedItem:{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{CustomModelData:3690013}}}] sword_ep 20
scoreboard players set @a[scores={give_energy=1..,sword_ep=..19},nbt={SelectedItem:{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{CustomModelData:3690013}}}] give_energy 0
#PORTAL
scoreboard players add @a[scores={take_energy=1..,portalprof=1..},nbt={SelectedItem:{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{CustomModelData:3690014}}}] sword_ep 20
scoreboard players remove @a[scores={take_energy=1..,portalprof=1..},nbt={SelectedItem:{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{CustomModelData:3690014}}}] portalprof 1
scoreboard players set @a[scores={take_energy=1..,portalprof=0},nbt={SelectedItem:{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{CustomModelData:3690014}}}] take_energy 0
scoreboard players add @a[scores={give_energy=1..,sword_ep=20..,portalprof=..1000},nbt={SelectedItem:{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{CustomModelData:3690014}}}] portalprof 1
scoreboard players remove @a[scores={give_energy=1..,sword_ep=20..},nbt={SelectedItem:{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{CustomModelData:3690014}}}] sword_ep 20
scoreboard players set @a[scores={give_energy=1..,sword_ep=..19},nbt={SelectedItem:{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{CustomModelData:3690014}}}] give_energy 0
#STAR
scoreboard players add @a[scores={take_energy=1..,starprof=1..},nbt={SelectedItem:{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{CustomModelData:3690015}}}] sword_ep 20
scoreboard players remove @a[scores={take_energy=1..,starprof=1..},nbt={SelectedItem:{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{CustomModelData:3690015}}}] starprof 1
scoreboard players set @a[scores={take_energy=1..,starprof=0},nbt={SelectedItem:{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{CustomModelData:3690015}}}] take_energy 0
scoreboard players add @a[scores={give_energy=1..,sword_ep=20..,starprof=..1000},nbt={SelectedItem:{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{CustomModelData:3690015}}}] starprof 1
scoreboard players remove @a[scores={give_energy=1..,sword_ep=20..},nbt={SelectedItem:{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{CustomModelData:3690015}}}] sword_ep 20
scoreboard players set @a[scores={give_energy=1..,sword_ep=..19},nbt={SelectedItem:{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{CustomModelData:3690015}}}] give_energy 0
#WOODEN
scoreboard players add @a[scores={take_energy=1..,woodenprof=1..},nbt={SelectedItem:{id:"minecraft:wooden_sword",Count:1b,tag:{swh:1b,CustomModelData:3690016}}}] sword_ep 20
scoreboard players remove @a[scores={take_energy=1..,woodenprof=1..},nbt={SelectedItem:{id:"minecraft:wooden_sword",Count:1b,tag:{swh:1b,CustomModelData:3690016}}}] woodenprof 1
scoreboard players set @a[scores={take_energy=1..,woodenprof=0},nbt={SelectedItem:{id:"minecraft:wooden_sword",Count:1b,tag:{swh:1b,CustomModelData:3690016}}}] take_energy 0
scoreboard players add @a[scores={give_energy=1..,sword_ep=20..,woodenprof=..1000},nbt={SelectedItem:{id:"minecraft:wooden_sword",Count:1b,tag:{swh:1b,CustomModelData:3690016}}}] woodenprof 1
scoreboard players remove @a[scores={give_energy=1..,sword_ep=20..},nbt={SelectedItem:{id:"minecraft:wooden_sword",Count:1b,tag:{swh:1b,CustomModelData:3690016}}}] sword_ep 20
scoreboard players set @a[scores={give_energy=1..,sword_ep=..19},nbt={SelectedItem:{id:"minecraft:wooden_sword",Count:1b,tag:{swh:1b,CustomModelData:3690016}}}] give_energy 0
#STONE
scoreboard players add @a[scores={take_energy=1..,stoneprof=1..},nbt={SelectedItem:{id:"minecraft:stone_sword",Count:1b,tag:{swh:1b,CustomModelData:3690017}}}] sword_ep 20
scoreboard players remove @a[scores={take_energy=1..,stoneprof=1..},nbt={SelectedItem:{id:"minecraft:stone_sword",Count:1b,tag:{swh:1b,CustomModelData:3690017}}}] stoneprof 1
scoreboard players set @a[scores={take_energy=1..,stoneprof=0},nbt={SelectedItem:{id:"minecraft:stone_sword",Count:1b,tag:{swh:1b,CustomModelData:3690017}}}] take_energy 0
scoreboard players add @a[scores={give_energy=1..,sword_ep=20..,stoneprof=..1000},nbt={SelectedItem:{id:"minecraft:stone_sword",Count:1b,tag:{swh:1b,CustomModelData:3690017}}}] stoneprof 1
scoreboard players remove @a[scores={give_energy=1..,sword_ep=20..},nbt={SelectedItem:{id:"minecraft:stone_sword",Count:1b,tag:{swh:1b,CustomModelData:3690017}}}] sword_ep 20
scoreboard players set @a[scores={give_energy=1..,sword_ep=..19},nbt={SelectedItem:{id:"minecraft:stone_sword",Count:1b,tag:{swh:1b,CustomModelData:3690017}}}] give_energy 0
#IRON
scoreboard players add @a[scores={take_energy=1..,ironprof=1..},nbt={SelectedItem:{id:"minecraft:iron_sword",Count:1b,tag:{swh:1b,CustomModelData:3690018}}}] sword_ep 20
scoreboard players remove @a[scores={take_energy=1..,ironprof=1..},nbt={SelectedItem:{id:"minecraft:iron_sword",Count:1b,tag:{swh:1b,CustomModelData:3690018}}}] ironprof 1
scoreboard players set @a[scores={take_energy=1..,ironprof=0},nbt={SelectedItem:{id:"minecraft:iron_sword",Count:1b,tag:{swh:1b,CustomModelData:3690018}}}] take_energy 0
scoreboard players add @a[scores={give_energy=1..,sword_ep=20..,ironprof=..1000},nbt={SelectedItem:{id:"minecraft:iron_sword",Count:1b,tag:{swh:1b,CustomModelData:3690018}}}] ironprof 1
scoreboard players remove @a[scores={give_energy=1..,sword_ep=20..},nbt={SelectedItem:{id:"minecraft:iron_sword",Count:1b,tag:{swh:1b,CustomModelData:3690018}}}] sword_ep 20
scoreboard players set @a[scores={give_energy=1..,sword_ep=..19},nbt={SelectedItem:{id:"minecraft:iron_sword",Count:1b,tag:{swh:1b,CustomModelData:3690018}}}] give_energy 0
#GOLD
scoreboard players add @a[scores={take_energy=1..,goldprof=1..},nbt={SelectedItem:{id:"minecraft:gold_sword",Count:1b,tag:{swh:1b,CustomModelData:3690019}}}] sword_ep 20
scoreboard players remove @a[scores={take_energy=1..,goldprof=1..},nbt={SelectedItem:{id:"minecraft:gold_sword",Count:1b,tag:{swh:1b,CustomModelData:3690019}}}] goldprof 1
scoreboard players set @a[scores={take_energy=1..,goldprof=0},nbt={SelectedItem:{id:"minecraft:gold_sword",Count:1b,tag:{swh:1b,CustomModelData:3690019}}}] take_energy 0
scoreboard players add @a[scores={give_energy=1..,sword_ep=20..,goldprof=..1000},nbt={SelectedItem:{id:"minecraft:gold_sword",Count:1b,tag:{swh:1b,CustomModelData:3690019}}}] goldprof 1
scoreboard players remove @a[scores={give_energy=1..,sword_ep=20..},nbt={SelectedItem:{id:"minecraft:gold_sword",Count:1b,tag:{swh:1b,CustomModelData:3690019}}}] sword_ep 20
scoreboard players set @a[scores={give_energy=1..,sword_ep=..19},nbt={SelectedItem:{id:"minecraft:gold_sword",Count:1b,tag:{swh:1b,CustomModelData:3690019}}}] give_energy 0
#DIAMOND
scoreboard players add @a[scores={take_energy=1..,diamondprof=1..},nbt={SelectedItem:{id:"minecraft:diamond_sword",Count:1b,tag:{swh:1b,CustomModelData:3690020}}}] sword_ep 20
scoreboard players remove @a[scores={take_energy=1..,diamondprof=1..},nbt={SelectedItem:{id:"minecraft:diamond_sword",Count:1b,tag:{swh:1b,CustomModelData:3690020}}}] diamondprof 1
scoreboard players set @a[scores={take_energy=1..,diamondprof=0},nbt={SelectedItem:{id:"minecraft:diamond_sword",Count:1b,tag:{swh:1b,CustomModelData:3690020}}}] take_energy 0
scoreboard players add @a[scores={give_energy=1..,sword_ep=20..,diamondprof=..1000},nbt={SelectedItem:{id:"minecraft:diamond_sword",Count:1b,tag:{swh:1b,CustomModelData:3690020}}}] diamondprof 1
scoreboard players remove @a[scores={give_energy=1..,sword_ep=20..},nbt={SelectedItem:{id:"minecraft:diamond_sword",Count:1b,tag:{swh:1b,CustomModelData:3690020}}}] sword_ep 20
scoreboard players set @a[scores={give_energy=1..,sword_ep=..19},nbt={SelectedItem:{id:"minecraft:diamond_sword",Count:1b,tag:{swh:1b,CustomModelData:3690020}}}] give_energy 0
#NETHERITE
scoreboard players add @a[scores={take_energy=1..,netheriteprof=1..},nbt={SelectedItem:{id:"minecraft:netherite_sword",Count:1b,tag:{swh:1b,CustomModelData:3690021}}}] sword_ep 20
scoreboard players remove @a[scores={take_energy=1..,netheriteprof=1..},nbt={SelectedItem:{id:"minecraft:netherite_sword",Count:1b,tag:{swh:1b,CustomModelData:3690021}}}] netheriteprof 1
scoreboard players set @a[scores={take_energy=1..,netheriteprof=0},nbt={SelectedItem:{id:"minecraft:netherite_sword",Count:1b,tag:{swh:1b,CustomModelData:3690021}}}] take_energy 0
scoreboard players add @a[scores={give_energy=1..,sword_ep=20..,netheriteprof=..1000},nbt={SelectedItem:{id:"minecraft:netherite_sword",Count:1b,tag:{swh:1b,CustomModelData:3690021}}}] netheriteprof 1
scoreboard players remove @a[scores={give_energy=1..,sword_ep=20..},nbt={SelectedItem:{id:"minecraft:netherite_sword",Count:1b,tag:{swh:1b,CustomModelData:3690021}}}] sword_ep 20
scoreboard players set @a[scores={give_energy=1..,sword_ep=..19},nbt={SelectedItem:{id:"minecraft:netherite_sword",Count:1b,tag:{swh:1b,CustomModelData:3690021}}}] give_energy 0
#Messages
execute as @a[tag=sword,scores={give_energy=1..}] run title @s[tag=sword] actionbar [{"text":"Imbuing Energy Points...","color":"aqua"}]
execute as @a[tag=sword,scores={take_energy=1..}] run title @s[tag=sword] actionbar [{"text":"Resetting Proficiency...","color":"gold"}]
